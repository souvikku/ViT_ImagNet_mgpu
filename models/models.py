...
class AdvVisionTransformer(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer= None, attacker=NoOpAttacker()):
...
#             norm_layer = lambda planes: GN(num_groups=2, num_channels=planes, eps=1e-6), **kwargs)
            norm_layer = lambda planes: GN_2(normalized_shape=planes), **kwargs)
#         print(model.state_dict.keys())
    elif norm == 'group_4':
        model = VisionTransformer(
            patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
            norm_layer = lambda planes: GN(num_groups=4, num_channels=planes, eps=1e-6), **kwargs)
    elif norm == 'batch':
        model = VisionTransformer(
            patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
            norm_layer = lambda planes: BN(num_features=planes, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), **kwargs)
...
@register_model
def deit_mid_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
